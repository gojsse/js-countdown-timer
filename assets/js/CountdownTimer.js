...
function buildClockHTML(clockIndex) {
  ...
  const isFlipClock  = (clockElement.getAttribute('data-is-flip-clock') === 'true') ? true : false;
...
function init() {
  ...
  for (let index = 0; index < clocks.length; index++) {
    ...
    if ((!clocks[index].getAttribute('data-end-time')) || !clocks[index]) {
      console.log('Clock at index ' + index + ' HTML element missing data-end-time attribute or value.');
      continue;
    }

    const flipAttribute = (clocks[index].getAttribute('data-is-flip-clock') === 'true') ? true : false;
